...
/*
 * 2. 시퀀스를 사용하는 방법
 * 시퀀스명.CURRVAL : 현재 시퀀스의 값(마지막으로 성공적으로 만들어진 NEXTVAL값)
 * 시퀀스명.NEXTVAL : 시퀀스값을 증가시킨 뒤 증가된 시퀀스의 값
 *                  기존 시퀀스 값에서 INCREMENT BY값만큼 증가한 값
 *                  (시퀀스명.CURRVAL + INCEREMENT BY 값(기본값 1))
 * 
...
-- NEXTVAL를 한 번 이라도 수행하지 않으면 CURRVAL을 사용할 수 없음
-- CURRVAL은 마지막에 성공적으로 만들어진 NEXTVAL를 보여주는 임시값

SELECT SEQ_EMPNO.NEXTVAL * 963 FROM DUAL;
SELECT * FROM USER_SEQUENCES;
-- LAST_NUMBER : 지금NEXTVAL하면 나오는 값
 ...
 * -- 객체 접근권한 부여하는 법
 * [ 표현법 ]
 * GRANT 권한종류(SELECT, INSERT, UPDATE, DELECT) ON 객체명(테이블명) TO 계정명;
 * 
 * * 시스템권한은 종류가 많음
