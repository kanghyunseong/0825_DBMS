 ...
 * - [단일열] 다중열 서브쿼리 : 서브쿼래 수행 결과가 여러 열일 때
...
-- 안준영 사원의 급여보다 더 많은 급여를 받는 사원들의 사원명, 급여
SELECT
       SALARY
  FROM 
       EMPLOYEE
...
-- EMP_NAME, DEPT_CODE, PHONE JOB_CODE, JOB_NAME
...
SELECT
 ...
 WHERE
   ...
   AND          
       EMP_NAME != '박수현';
-------------------------------------------------------------------------------
-- 부서별로 급여 합계 (SUM) 가 가장 큰 부서의 부서명, 부서코드, 급여합계
-- 1. 각 부서별 급여 합계
SELECT * FROM EMPLOYEE;
...
SELECT * FROM EMPLOYEE;
-- 대리 직금임에도 불구하고 과장보다 급여를 많이 받는 대리가 존재한다.
 ...
 * 3. 다중 열 서브쿼리
 * 
 * 조회결과는 한 행이지만 나열된 컬럼의 수가 다수개일 떄
 */
SELECT * FROM EMPLOYEE;
SELECT * FROM JOB;
-- 박채형 사원과 같은 부서코드, 같은 직급코드에 해당하는 사원들 사원명, 부서코드, 직급코드 조회
SELECT
       DEPT_CODE
     , JOB_CODE
  FROM
 ...
 * 메인 쿼리 실행 마다 서브쿼리가 실행될 수 있으므로 성능이슈가 생길 수 있음
 * 그렇기 떄문에 JOIN으로 대체하는 것이 일반적으로는 성능상 유리함
 * 단, 캐싱이 되기 떄문에 동일한 결과에 대해선 성능상 JOIN보다 뛰어날 수도 있음
...
--> TOP-N 분석 (TOP에서 N개) : DB상에 있는 값들 중 최상위 N개의 데이터를 보기 위해서 사용
SELECT * FROM EMPLOYEE;
...
SELECT
 ...
 ORDER
    BY
       SALARY DESC
OFFSET 0 ROWS FETCH NEXT 5 ROWS ONLY; -- 0개 건너뛰고 5개 반환
