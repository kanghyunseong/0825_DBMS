 ...
 *			(RIGHT OUTER) 		|			전체 외부조인 (FILL OUTER JOIN) -> 오라클에선 X
 * ------------------------------------------------------------------------------------
 * 			카테시안 곱			| 		   교차조인
 * 		(CARTESIAN PRODUCT)		| 		(CROSS JOIN)
 * ------------------------------------------------------------------------------------
 * 자체조인 (SELF JOIN)
 * 비등가조인 (NON EQAUL JOIN)
 * 자연조인 (NATURAL JOIN)
 */
...
-- EMP_NAME, EDPT_CODE, DEPT_TITLE
-- 사원명, 부서코드, 부서명
-- 연결컬럼이 컬럼명이 다름(EMPLOYEE - DEPT_CODE / DEPARTMENT - DEPT_ID)
-- 무조건 ON구문만 사용가능( USING은 못 씀 안됨 불가능함)
SELECT
       EMP_NAME
     , DEPT_CODE
     , DEPT_TITLE
...
-- ON 구문이용) 애매해다(AMBIGUOISLY) 발생할 수 있음 어떤 테이블의 컬럼인지 명시
SELECT 
       EMP_NAME
     , JOB_NAME
  FROM
 ...
 * 
 */
-- 사원명과 직급명을 같인 조회해주세요. 단 직급명이 대리인 사원들만 조회해주세요
SELECT * FROM EMPLOYEE;
SELECT * FROM JOB;
 ...
 * 단, 반드시, LEFT / RIGTH를 지정해줘야함!(기준 테이블을 선택해야함)
 */
...
--INNER  
   JOIN
	    DEPARTMENT ON (DEPT_CODE = DEPT_ID);  
-- EMLOYEE 테이블에서 DEPT_CODE가 NULL인 두 명의 사원은 조회X
...
--------------------------------------------------------------
/*
 * 카테시안 곱(CRETESIAN PRODUCT) / 교차조인(CROSS JOIN)
...
SELECT * FROM LOCATION;     -- LOCAL_NAME             				LOCALCODE
