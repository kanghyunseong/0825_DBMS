...
-- 4. EMEPLOYEE 테이블에서 EMP_NAME, EMAIL, PHONE컬럼을 조회하세요.
...
-- 5. EMEPLOYEE 테이블에서 HIRE_DATE, EMP_NAME, SALARY컬럼을 조회하세요.
...
/*
 * < 컬럼의 조회된 값을 통한 산술연산 >
 * 컬럼들을 나열해서 조회를 할 떄 산술연산 (+, -, *, /)을 기술해서 결괄르 조회할 수 있음
 */
...
-- EMPLOYEE테이블로부터 사원명, 월급, 보너스를 포함한 연봉((월급 * 보너스 + 월급) * 12)
 ...
 * 
 * 별칭에 특수문자 또는 공백문자가 포함될 경우 ""로 묶어주어야함
 */
...
-- EMPLOYEE테이블로부터 사원들의 사원명, 급여 조회 --> 급여가 300 만원 이상인 사원들만
SELECT 
   	   EMP_NAME
   	 , SALARY
  FROM 		
...
-- EMPLOYEE 테이블로부터 급여 컬럼의 값이 350만원 이상이고 500만원 이하인 사원들의
-- 사번(EMP_ID), 이름, 급여를 조회
...
/*
 * < BETWEEN AND>
 * 몇 이상 몇 이하인 범위에 대해서 조건을 제시할 떄 사용
 * 
 * [ 표현법 ]
...
-- 이름의 두 번쨰 글자가 '승'인 사원들의 사원명, 핸드폰번호
SELECT
       EMP_NAME
     , PHONE
  FROM 
...
SELECT 
  ...
  FROM
       EMPLOYEE
 WHERE
      -- BONUS IS NULL; -- (NULL인경우)
	   BONUS IS NOT NULL; -- (NULL이 아닌경우)
--------------------------------------------------------------------------
/*
 * < 연결연산자 || >
 * 여러개의 컬럼 값들을 마치 하나의 컬럼인것처럼 연결시켜주는 연산자
 * 컬럼값 또는 리터럴(문자열)을 전부 다 합칠 수 있음
 * System.out.println(num + "sdasd");
 * 이런느낌
 * 
 * [ 표현법 ]
 */
 ...
 * 
 */
-- EMPLOYEE 테이블로부터 부서코드가 D6 이거나 D8이거나 D5인 사원들의 사원명, 부서코드
SELECT 
       EMP_NAME
     , DEPT_CODE
  FROM
 ...
 * ORDER BY 절을 작성하지 않으면 ResultSet은 정렬이 안된 상태
 * 
 * [ 표현법 ]
 * SELECT
 *        컬럼명
 ...
 *  ORDER
 *     BY
 *        [정렬기준으로 삼고싶은 컬럼명 / 별칭 / 칼럼 순번 ]
 * 		  [ASC / DESC]
 *        [NULL FIRST / NULL LAST] (생략가능)
 * - ASC : 오름차순 정렬(기본값)
 * - DESC : 내림차순 정렬
 * - NULLS FIRST : 칼럼값이 NULL일 경우 조회결과의 위쪽에 배치(내림차순일 경우 기본값)
 * - NULLS LAST : 컬럼값이 NULL일 경우 조회결과의 아래쪽에 배치(오름차순을 경우 기본값)
 */
